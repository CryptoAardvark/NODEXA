...
template int Equihash<255, 11>::InitialiseState(eh_HashState& base_state, bool btg_salt);
template bool Equihash<255, 11>::BasicSolve(const eh_HashState& base_state,
    const std::function<bool(std::vector<unsigned char>)> validBlock,
    const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<255, 11>::OptimisedSolve(const eh_HashState& base_state,
    const std::function<bool(std::vector<unsigned char>)> validBlock,
    const std::function<bool(EhSolverCancelCheck)> cancelled);
template bool Equihash<255, 11>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln);
