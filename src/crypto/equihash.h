...
static Equihash<255, 11> Eh255_11;

#define EhInitialiseState(n, k, base_state, btg_salt)  \
    if (n == 255 && k == 11) {                           \
        Eh255_11.InitialiseState(base_state, btg_salt);  \
    }  else {                                           \
...
{
    if (n == 255 && k == 11) {
        return Eh255_11.BasicSolve(base_state, validBlock, cancelled);
    } else {
        throw std::invalid_argument("Unsupported Equihash parameters");
    }
}
...
{
    if (n == 255 && k == 11) {
        return Eh255_11.OptimisedSolve(base_state, validBlock, cancelled);
    } else {
        throw std::invalid_argument("Unsupported Equihash parameters");
    }
}
...
#define EhIsValidSolution(n, k, base_state, soln, ret)   \
    if (n == 255 && k == 11) {                             \
        ret = Eh255_11.IsValidSolution(base_state, soln);  \
    } else {                                             \
        throw std::invalid_argument("Unsupported Equihash parameters"); \
    }
